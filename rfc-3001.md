...
Publication authority is enforced by DNS delegation — the entity controlling the authoritative DNS zone for the UniKey domain controls key publication. Keys MUST carry explicit validity periods. For high-assurance deployments, the maximum validity period is 90 days. Rotation MUST occur without gap: the successor key MUST be published in DNS before the predecessor key expires. Revocation is effected by DNS record removal and is effective upon DNS propagation. High-assurance deployments SHOULD additionally publish a revocation endpoint to support real-time revocation checking independent of DNS TTL.
