...
The device signs the canonical Trust Packet Hash (TPH) defined in RFC 2001.
...
3. Generate the Trust Packet Hash (TPH) as defined in RFC 2001.
...
## 9. Device Binding and Attestation
...
1. The device binding credential MUST be non-exportable from the device or SIM.
2. The approval signal MUST be authenticated to the UniKey cloud service before the Trust Packet is signed.
3. The binding MUST be traceable to a specific subscriber, token holder, or enrolled device. Unbound credentials MUST NOT be accepted as Trust Packet originators.
...
## 10. Delegated Device Authority

Devices MAY delegate limited authority to trusted agents or applications.

Delegation MUST follow the Trust Packet delegation rules defined in RFC 2001 and RFC 1200.
...
## 11. Device Registration and Key Discovery
...
When implemented according to this specification and RFC 2001, device authority provides the following properties:
...
## 13. Relationship to Other Security Models
...
- signs the Trust Packet Hash as defined in RFC 2001
